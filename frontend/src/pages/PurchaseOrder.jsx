...
export default function PurchaseOrder() {
  ...
  const [customerId, setCustomerId] = useState(null); const [statusFilter, setStatusFilter] = useState(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const r = await purchaseOrderApi.list({ page: page-1, size: PAGE_SIZE, customerId: customerId||undefined, status: statusFilter||undefined }); setData(r.data.data.content||[]); setTotal(r.data.data.totalElements||0); }
    catch { message.error('データの取得に失敗しました'); } finally { setLoading(false); }
  }, [page, customerId, statusFilter]);
